// Run with node tests/currency-details.test.cjs. Simulated DOM/storage, not browser QA.
...
function runtime(version) {
  ...
  const expose = `window.test={state,renderCurrencies,handleBound,installEvents,saveNow,spreadHtml,
    relativeRepricingResult,expectedPolicyDifferentialResult${version !== '5.4.7' ? ',currencySummaryHtml' : ''}${version === '5.5.3' ? ',currencyDocumentationStatus,CURRENCY_DOCUMENTATION_FIELDS,currencyDetailOpen' : ''}};`;
...
const old = runtime('5.5.2'), current = runtime('5.5.3');
...
async function main() {
  ...
  assert.equal((initial.match(/<details\b/g)||[]).length,2);
  ...
  console.log('PASS: all eight currencies retain identical controls/options/values; collapsed defaults, five metadata groups, independent disclosure state, toggle purity and status updates verified; USD/EUR/JPY edits, tab clicks, summary refresh, serialized records/assets and rate calculations verified in simulated DOM/storage.');
}

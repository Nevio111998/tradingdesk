...
for(const pair of ['EURUSD','GBPUSD','USDCHF']){
 ...
 for(const api of [rt,old])Object.assign(api.state,{records:[m,t],record:t,id:t.id,route:'trade',tab:'overview'});
 const before=JSON.stringify(t),prior=allViews(old,t),after=allViews(rt,t);
 assert.equal(JSON.stringify(t),before);assert.deepEqual([...new Set(bindings(after))].sort(),[...new Set(bindings(prior))].sort(),'All stored fields remain accessible across views');
 const checks=h=>[...h.matchAll(/data-check="([^"]+)"/g)].map(x=>x[1]);assert.deepEqual(checks(after),checks(prior));assert.equal(checks(after).length,17);
 assert.equal(JSON.stringify(rt.readiness(t)),JSON.stringify(old.readiness(t)));assert.equal(String(rt.relativeRepricingResult),String(old.relativeRepricingResult));
 ...
 assert(bindings(after).length<bindings(prior).length);
...
console.log('PASS: all fields retained across trade views; 17 checks, 3 pairs, frozen snapshot, details isolation, persistence, navigation and rates/readiness unchanged.');

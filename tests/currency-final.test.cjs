...
function runtime(version) {
  ...
  const expose = `window.test={state,renderCurrencies,handleBound,installEvents,saveNow,spreadHtml,
    relativeRepricingResult,expectedPolicyDifferentialResult${version !== '5.4.7' ? ',currencySummaryHtml' : ''}${['5.5.3','5.5.4','5.5.5'].includes(version) ? ',currencyDocumentationStatus,CURRENCY_DOCUMENTATION_FIELDS,currencyDetailOpen' : ''}${['5.5.4','5.5.5'].includes(version) ? ',currencyOverviewHtml,currencyUpcomingEvents,currencyWatchHtml' : ''}};`;
...
const old = runtime('5.5.4'), current = runtime('5.5.5');
...
async function main() {
  ...
  select(current, after, 'JPY');
  current.handleBound({dataset:{bind:'currencies.JPY.yield2'},value:'1.1',type:'number'});
  assert.equal(current.element('#macro-spread').innerHTML,current.spreadHtml(after),'Yield edit must immediately refresh pair comparison');
  assert(current.element('#macro-spread').innerHTML.includes('+190.0 bp'));

...
// Pair-only checklist configuration. Shared trade definitions and all saved
// records remain unchanged. Preserve the original IDs for existing checks.
const PAIR_CHECK_SELECTION = {
  "event": [
    "calendarToday",
    "speeches"
  ],
...
const pairMacroGroups = groups.filter(g=>Object.hasOwn(PAIR_CHECK_SELECTION,g.id)).map((g,index)=>{
  const selected=new Set(PAIR_CHECK_SELECTION[g.id]);
  const items=g.items.filter(item=>selected.has(item.id)).map(item=>({...item}));
  const sourceLinks=[...new Set(g.items.flatMap(item=>item.links||[]))];
  if(items.length!==selected.size)throw new Error('Unbekannte Paar-Checklisten-ID: '+g.id);
  if(g.id==='event')items.find(item=>item.id==='calendarToday').label='Wirtschaftskalender vollständig geprüft';
  if(g.id==='policy'){
    items.find(item=>item.id==='policyLatest').label='Letzte Entscheidung und Kommunikation geprüft';
    items.find(item=>item.id==='policyTone').label='Tonänderung gegenüber vorherigen Reden / Kommunikation geprüft';
    items.find(item=>item.id==='policyTone').help='Hawkish/dovish relativ zur vorherigen Kommunikation und zu den Markterwartungen beurteilen.';
  }
  if(g.id==='rates'){
    const change=items.find(item=>item.id==='ratesChange');
    change.label='Veränderung 1 Tag / 5 Tage / 14 Tage geprüft';
    change.help='Vergleiche die 2Y-Renditen und das relative Differential für 1, 5 und 14 Tage anhand tatsächlicher Datenstände. Nicht nur das Niveau betrachten; ausweitende oder verengende Differenzen einordnen.';
  }
  if(g.id==='positioning')items.find(item=>item.id==='cotLatest').label='Aktuelles COT geprüft';
  return {...g,number:String(index+1).padStart(2,'0'),items,sourceLinks};
});


// Trade-only presentation definitions. Never remove historic keys from records.
// The original groups remain available for legacy data and archived snapshots.
const TRADE_CHECK_SELECTION = {
  "event": [
    "calendarToday"
  ],
  "edge": [
...
window.FX_COMPANION_DATA_VERSION='5.6.9';

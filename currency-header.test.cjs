// Run with node tests/currency-header.test.cjs. Simulated DOM/storage, not browser QA.
...
function runtime(version) {
  ...
  const expose = `window.test={state,renderCurrencies,handleBound,installEvents,saveNow,spreadHtml,
    relativeRepricingResult,expectedPolicyDifferentialResult${version === '5.5.1' ? ',currencySummaryHtml' : ''}};`;
...
const old = runtime('5.4.7'), current = runtime('5.5.1');
...
for (const currency of ['USD', 'EUR', 'JPY', 'GBP', 'AUD', 'NZD', 'CAD', 'CHF']) {
  ...
  // The entire original form, metadata, calculator and sidebar remain byte-identical.
  assert.equal(updatedHtml.slice(updatedHtml.indexOf('<div class="panel-grid">')),
    legacyHtml.slice(legacyHtml.indexOf('<div class="panel-grid">')));
...
async function main() {
  ...
  console.log('PASS: all eight currencies retain identical forms; USD/EUR/JPY edits, tab clicks, summary refresh, serialized records/assets and rate calculations verified in simulated DOM/storage.');
}
